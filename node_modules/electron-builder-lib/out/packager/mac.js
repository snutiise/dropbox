...
exports.createMacApp = undefined;
...
/** @internal */
let createMacApp = exports.createMacApp = (() => {
    var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (packager, appOutDir, asarIntegrity) {
        ...
        if (oldHelperBundleId != null) {
            (_builderUtil || _load_builderUtil()).log.warn("build.helper-bundle-id is deprecated, please set as build.mac.helperBundleId");
        }
        ...
        const protocols = (0, (_builderUtil || _load_builderUtil()).asArray)(buildMetadata.protocols).concat((0, (_builderUtil || _load_builderUtil()).asArray)(packager.platformSpecificBuildOptions.protocols));
        if (protocols.length > 0) {
            appPlist.CFBundleURLTypes = protocols.map(function (protocol) {
                const schemes = (0, (_builderUtil || _load_builderUtil()).asArray)(protocol.schemes);
                if (schemes.length === 0) {
        ...
        if (fileAssociations.length > 0) {
            appPlist.CFBundleDocumentTypes = yield (_bluebirdLst2 || _load_bluebirdLst2()).default.map(fileAssociations, (() => {
                var _ref2 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (fileAssociation) {
                    const extensions = (0, (_builderUtil || _load_builderUtil()).asArray)(fileAssociation.ext).map((_platformPackager || _load_platformPackager()).normalizeExt);
                    const customIcon = yield packager.getResource((0, (_builderUtil || _load_builderUtil()).getPlatformIconFileName)(fileAssociation.icon, true), `${extensions[0]}.icns`);
        ...
        (0, (_builderUtil || _load_builderUtil()).use)(packager.platformSpecificBuildOptions.category || buildMetadata.category, function (it) {
